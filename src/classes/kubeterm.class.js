        ...
        if (opts.role === "client") {
            ...
            this._getTtyCTX = tty => {
                return new Promise((resolve, reject) => {
                    require('child_process').exec('kubectl config current-context', (err, stdout, stderr) => {
                        if (err) {
                            reject("Could not get the current context: "+stderr);
                        }
                        console.log(stdout);
                        resolve(stdout);
                    });
            ...
            this._getTtyNS = tty =>  {
                return new Promise((resolve, reject) => {
                    require('child_process').exec('kubectl config view --minify --output \'jsonpath={..namespace}\'', (err, stdout, stderr) => {
                        if (err) {
                            reject("Could not get the current namespace: "+stderr);
                        }
                        resolve(stdout);
                    });   
                });
            ...
            this._nextTickUpdateTtyCTX = false;
            this._nextTickUpdateTtyNS = false;
            this._tick = setInterval(() => {
                if (this.renderer && this._nextTickUpdateTtyCTX) {
                    this._nextTickUpdateTtyCTX = false;
                    this._getTtyCTX(this.tty).then(ctx => {
                        if (this.tty._ctx === ctx) return;
                        this.tty._ctx = ctx;
                        if (this.renderer) {
                            this.renderer.send("terminal_channel-"+this.port, "New ctx", ctx);
                        }
                    }).catch(e => {
                        if (!this._closed) {
                            console.log("Error while tracking TTY current context: ", e);
                            try {
                                this.renderer.send("terminal_channel-"+this.port, "Fallback ctx", opts.ctx);
                            } catch(e) {
                                // renderer closed
                            }
                        }
            ...
            require('child_process').exec('kubectl config current-context', (err, stdout, stderr) => {
                if (err) {
                    const first_context=stderr;
                } else {
                    const first_context=stdout;
                }
            });
        
            require('child_process').exec('kubectl config view --minify --output \'jsonpath={..namespace}\'', (err, stdout, stderr) => {
                if (err) {
                    first_namespace=stderr;
                } else {
                    first_namespace=stdout;
                }
            });   
            ...
            this.wss.on("connection", ws => {
                ...
                this.tty.on("data", data => {
                    ...
                    this._nextTickUpdateTtyCTX = true;
                    this._nextTickUpdateTtyNS = true;
                    try {
                        ws.send(data);
                    } catch (e) {
                        // Websocket closed
